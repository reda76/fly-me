...
def result_to_json(result):
    try:
        ...
        luis_result = re.sub(r">", ">'", luis_result)
...
def extract(result):
    ...
    try:
        ...
        for inf_fly in list_information_fly:
            if inf_fly == "budget":
                try:
                    dict_information[inf_fly] = parse_price(
                        dict_fly[inf_fly][0])
                except:
                    dict_information[inf_fly] = "None"
            elif inf_fly == "str_date":
                try:
                    dict_information[inf_fly] = parse_date(
                        dict_fly[inf_fly][0]).date()
                except:
                    dict_information[inf_fly] = "None"
    ...
    try:
        dict_information["money"] = float(
            result["entities"]["money"][0]['number'])
    except:
    ...
    if dict_information["end_date"] == 'None':
        if type(dict_information['datetimeV2']) is list:
            try:
                dict_information["end_date"] = dict_information['datetimeV2'][
                    1]
            except:
                pass
...
def message_si_manque_info(dict_information):
    ...
    dict_info_sans_prebuilt = {
        "your origine city": origine,
        "your destination city": destination,
        "your start date": start_date,
        "your end date": end_date,
        "your budget": budget
    }
    liste_info_manque = []
