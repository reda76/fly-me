...
logger.addHandler(
    AzureLogHandler(connection_string='InstrumentationKey=' +
                    key_insight_instrumentation))


class LuisBot(ActivityHandler):

    def __init__(self, conversation: ConversationState):
        luis_app = LuisApplication(app_id, appPrediction_Key,
                                   endpointPrediction_url)
        luis_option = LuisRecognizerOptionsV3(include_all_intents=False,
        ...
        self.dialog_set.add(
            WaterfallDialog("main_dialog", [
                self.GetBooking, self.Verification, self.VerificationDeux,
                self.VerificationTrois
            ]))
    ...
    async def GetBooking(self, waterfall_step: WaterfallStepContext):
        ...
        return await waterfall_step.prompt(
            "text_prompt", PromptOptions(prompt=MessageFactory.text(message)))

    async def Verification(
            self, waterfall_step: WaterfallStepContext) -> DialogTurnResult:
    ...
    async def VerificationDeux(
        ...
        waterfall_step.values["dict_extract"] = dict_extract
        if luis_result_2["text"] in [
                'No', "no", "Nop", "nop", "Absolutely no", "Absolutely No",
            ...
            properties = {
                'custom_dimensions': {
                    'turn': str([luis_result, luis_result_2]),
                    'information': str(dict_extract),
                    'reason':
                    "The bot to propose a reservation that does not comply with the user's request",
                    'history': str(waterfall_step.values["history"])
                }
            }
            logger.warning('action', extra=properties)
        elif luis_result_2["text"] not in [
    ...
    async def VerificationTrois(
        ...
        if luis_result_3["text"] in [
            ...
            properties = {
                'custom_dimensions': {
                    'turn': str([luis_result, luis_result_2, luis_result_3]),
                    'information': str(dict_extract),
                    'reason':
                    "The bot to propose a reservation that does not comply with the user's request",
                    'history': str(waterfall_step.values["history"])
            ...
            logger.warning('action', extra=properties)
        elif luis_result_3["text"] not in [
                'Yes', "yes", "yep", "Yep", "yeah", "Yeah"
        ]:
            count = 0
            for key, value in dict_extract.items():
            ...
            if count != 0:
                ...
                properties = {
                    'custom_dimensions': {
                        'turn':
                        str([luis_result, luis_result_2, luis_result_3]),
                        'information': str(dict_extract),
                        'reason':
                        "The bot failed to understand the user's request after the 3rd exchange",
                        'history': str(waterfall_step.values["history"])
                    }
                }
                logger.warning('action', extra=properties)
